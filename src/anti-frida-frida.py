...
if __name__ == "__main__":
    ...
    all_patch_string = ["FridaScriptEngine", "GLib-GIO", "GDBusProxy", "GumScript"] # 字符串特征修改 尽量与源字符一样 # google translate 'Modify string characteristics and try to be the same as the source characters'
    for section in binary.sections:
        if section.name != ".rodata":
            continue
        for patch_str in all_patch_string:
            addr_all = section.search_all(patch_str) # Patch 内存字符串 # google translate 'Patch memory string'
